...
Процедура ПечатьРаздела1(ДокументРезультат)
	...
	ВыборкаЗаписей = РегистрыНакопления.КнигаУчетаДоходовИРасходов.Выбрать(НачДата, КонецДня(Период), Новый Структура("Организация", Организация));
	Пока ВыборкаЗаписей.Следующий() Цикл
		
		Если НЕ ВыборкаЗаписей.Активность Тогда
			Продолжить;
		КонецЕсли;
