...
Процедура ВыполнитьРегистрациюИзмененийПоУзлуИМетаданному(УзелДляРегистрацииИзменений, МетаданныеДляОбмена, 
	...
	Если (НЕ РегистрироватьВсеИзменения)
		...
		ВозможныОграниченияВыгрузкиСчета = (МетаданныеДляОбмена = Метаданные.Документы.СчетНаОплатуПокупателю);
	...
	ИначеЕсли (НЕ РегистрироватьВсеИзменения)
		...
		Если ПодключениеКПриемнику <> Неопределено Тогда
			...
			ВозможныОграниченияВыгрузкиСчета = (МетаданныеДляОбмена = ПодключениеКПриемнику.Метаданные().Документы.СчетНаОплатуПокупателю ИЛИ МетаданныеДляОбмена = ПодключениеКПриемнику.Метаданные().Документы.ЗаказПокупателя);
			...
			ВозможныОрганиченияПоСкладу = УзелДляРегистрацииИзменений.Склады.Количество() > 0 И ЭтоДокумент;
	
			ВозможныОрганиченияПоПодразделению = УзелДляРегистрацииИзменений.Подразделения.Количество() > 0 И ЭтоДокумент;
	
			ВозможныОграниченияПоОтражатьВБухгалтерскомУчете =  (МетаданныеДляОбмена.Реквизиты.Найти("ОтражатьВБухгалтерскомУчете") <> Неопределено) И ЭтоДокумент И УзелДляРегистрацииИзменений.ПередаватьТолькоДокументыОтраженныеВБухгалтерскомУчете;
			...
			Если НаличиеСклада Тогда
				...
				Запрос.УстановитьПараметр("МассивСкладов", УзелДляРегистрацииИзменений.Склады.Выгрузить().ВыгрузитьКолонку("Склад"));
			...
			Если НаличиеПодразделения Тогда
				...
				Запрос.УстановитьПараметр("МассивПодразделений", УзелДляРегистрацииИзменений.Подразделения.Выгрузить().ВыгрузитьКолонку("Подразделение"));
			...
			Если ВозможныОграниченияВыгрузкиСчета Тогда
				...
				ИначеЕсли УзелДляРегистрацииИзменений.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю = ПодключениеКПриемнику.Перечисления.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю.ЗаказПокупателя
					И МетаданныеДляОбмена = ПодключениеКПриемнику.Метаданные().Документы.СчетНаОплатуПокупателю  Тогда
					
					ОграничениеВыборкиПоВыгрузкеСчета = "ЛОЖЬ";	
					
				ИначеЕсли УзелДляРегистрацииИзменений.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю = ПодключениеКПриемнику.Перечисления.ВыгрузкаДокументовЗаказПокупателяСчетНаОплатуПокупателю.СчетНаОплатуПокупателю
					И МетаданныеДляОбмена = ПодключениеКПриемнику.Метаданные().Документы.ЗаказПокупателя Тогда
...
Функция ОпределитьНаличиеСкладаВМетаданныхДляРегистрации(МетаданныеОбъекта)
	...
	Если МетаданныеОбъекта = ПодключениеКПриемнику.Метаданные().Документы.АвансовыйОтчет
		ИЛИ МетаданныеОбъекта =  ПодключениеКПриемнику.Метаданные().Документы.ВозвратТоваровОтПокупателя
		ИЛИ МетаданныеОбъекта =  ПодключениеКПриемнику.Метаданные().Документы.ПоступлениеТоваровУслуг  Тогда
		
		МассивРеквизитов.Добавить("СкладОрдер");
		
	ИначеЕсли МетаданныеОбъекта =  ПодключениеКПриемнику.Метаданные().Документы.ЗаказПокупателя Тогда
		
		МассивРеквизитов.Добавить("СкладГруппа");	
			
	ИначеЕсли МетаданныеОбъекта =  ПодключениеКПриемнику.Метаданные().Документы.ПеремещениеТоваров  Тогда
