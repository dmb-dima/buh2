...
Процедура СохранитьПараметрыТабличногоДокумента(Результат, ВариантОтчета) Экспорт
	...
	СохранитьЗначение("УнивесальныйОтчет_НастройкиВариантаОтчета_" + ВариантОтчета.НастраиваемыйОбъект + "/" + ВариантОтчета, Структура);
КонецПроцедуры
...
Процедура ОбработкаФормыПослеПримененияНастройки(ОтчетОбъект, ФормаОтчета, ИспользоватьЗначенияНастроекНаФорме = Ложь) Экспорт
	...
	ТиповыеОтчеты.УправлениеОтображениемПанелиПользователя(ФормаОтчета);
...
Функция ЗаписьОтчетаДоступна(ОтчетОбъект, ПроверяемаяСохраненнаяНастройка = Неопределено) Экспорт
	...
	СписокДоступныхНастроек = ТиповыеОтчеты.ПолучитьСписокДоступныхВариантов(ТиповыеОтчеты.ПолучитьИдентификаторОбъекта(ОтчетОбъект), глЗначениеПеременной("глТекущийПользователь"),, ОтчетОбъект);
...
Процедура ОткрытьФормуРасшифровки(ОтчетОбъект, Заголовок) Экспорт
	...
	ТиповыеОтчеты.УстановитьПараметрВывода(ОтчетОбъект.КомпоновщикНастроек, "Title", Заголовок);
