...
Процедура СброситьУстановленныйКодНомерОбъекта(ЭтотОбъект, КодНомер, ПодменюДействия, ПолеВводаНомера) Экспорт
	...
	Иначе
		...
		Если ОбъектМетаданные.Реквизиты.Найти("ПодразделениеОрганизации") <> Неопределено Тогда
			ПрефиксПодразделения = ЭтотОбъект.ПодразделениеОрганизации.Префикс;
			ДлинаПрефиксаПодразделения = СтрДлина(ПрефиксПодразделения);
			Если ДлинаПрефиксаПодразделения = 0 Тогда
				ЭтотОбъект[КодНомер] = "";
			Иначе
				НовыйПрефикс = НовыйПрефикс + ПрефиксПодразделения;
			КонецЕсли;
		ИначеЕсли ОбъектМетаданные.Реквизиты.Найти("ПодразделениеОтправитель") <> Неопределено Тогда
			ПрефиксПодразделения = ЭтотОбъект.ПодразделениеОтправитель.Префикс;
			ДлинаПрефиксаПодразделения = СтрДлина(ПрефиксПодразделения);
			Если ДлинаПрефиксаПодразделения = 0 Тогда
				ЭтотОбъект[КодНомер] = "";
			Иначе
